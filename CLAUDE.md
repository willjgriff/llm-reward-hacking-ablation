...
- Unattended Claude Code on the box (user decision 2026-09-20): for the models × benchmarks transcript sweep, Claude Code runs on the box as **root** with `IS_SANDBOX=1 claude --dangerously-skip-permissions` in tmux `claude`, logged in with the subscription (`/login`; `/logout` when done, the stopped disk keeps the token). All code for it is in `sweep/`: `box_claude.sh` (install + launch + watchdog), `CLAUDE.unattended.md` (copied to `/root/.claude/CLAUDE.md`; box-only rules that override this file: no plan mode or approval stops, all model-written code through `rhbench-run`, no credentials reachable by `rhbench`, reports in `box_report/LOG.md` + `SUMMARY.md`, uploads under `sweep/<model>/<benchmark>/` via `upload_run.sh --dest-prefix`), `mission.md` (the prompt), `upload_code.sh` (box → HF `sweep/code/`), `pull_box.sh` (laptop). Code leaves the box only via `sweep/upload_code.sh` → `sweep/code/` in the private HF dataset repo (user decision 2026-09-21; mirror of the code tree, one commit per upload, refuses files containing credentials); the GitHub repo is unreachable from the box (no `.git`, no GitHub credentials, git forbidden), and run data folders stay code-free (`upload_run.sh` refuses `.py`/`.sh`/`.ipynb`). On the laptop `sweep/pull_box.sh [--from-hf <repo>]` puts that code in the quarantine directory; it is untrusted either way. Watchdog = `stop_box_when_idle.sh --idle-minutes 120`; an interactive `claude` process is deliberately not "busy" (a stalled or finished session must not hold the box up), while `hf download`, `uv sync/pip` and `docker pull/build` are. Laptop safety: no VS Code Remote-SSH to the box, never rsync the box tree over the working tree (code comes back into `~/rhablation-box-quarantine/` and is read as a diff), no unpickling of box artefacts; details in `setup.md`.
